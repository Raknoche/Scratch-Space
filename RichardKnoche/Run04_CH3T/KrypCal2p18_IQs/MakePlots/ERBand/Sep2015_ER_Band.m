...
%% Make plot of wall events versus radius
% 
% i=1;
% for max_r=5:1:25;
%     fiducial_cut=inrange(drift_time,[40 300]) & inrange(s2radius_del,[0,max_r]);
%     tritium_cut= clean_cut & pulse_area_cut & fiducial_cut;
%     tritium_cut_noarea= clean_cut & fiducial_cut;
%     num_events(i)=length(s1_phe_both_xyz(tritium_cut & inrange(s1_phe_both_xyz([0 50]));
%     num_events_noarea(i)=length(s1_phe_both_xyz(tritium_cut_noarea & inrange(s1_phe_both_xyz([0 50]));
%     num_wall_events(i)=num_events(i)-num_events_noarea(i);
%     num_wall_events_err(i)=sqrt( num_events(i) + num_wall_events(i));
%     radius_marker(i)=max_r;
%     i=i+1;
% end
...
%             temporary, for calculating correct E-lifetime   
 ...
 density(s1_phe_both_xyz(tritium_cut),log10(s2_phe_both_xyz(tritium_cut)./s1_phe_both_xyz(tritium_cut)),[0:.2:100],[0:.02:4]);
...
%  plot(ER_bins,upper_bound,'ok','markersize',6,'linewidth',2)
 ...
 box;
  ...
  %Save ER Band fits
  save('Sep2015_2p18_ERFit','ER_bins','mean_ER_band','ER_sigma','ER_lower_power_fit','ER_upper_power_fit','ER_mean_power_fit','ER_lower_fit','ER_upper_fit','ER_mean_fit');
