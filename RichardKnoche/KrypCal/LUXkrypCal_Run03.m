...
Pulse_Class=1; % was a relic from the days of no pulse clasifer (can set to 0 and use prompt fraction cut)
...
%if ~exist('calibrations','var') || isempty(calibrations)
%    calibrations = [1 1 1 1 1 1];
%    fprintf('Calibration selection missing (defaulting to all)...\n');
%end
...
%if ~exist('drift_V','var') || isempty(submit)
%    drift_V = 1.51; %\mus/us at the standard 182 V/cm field
%    fprintf('Drift velocity defult to 1.51 \mus/us (assuming 182 V/cm)..\n');
%end



%if ~exist('file_id','var') || isempty(file_id)
%    if isfield(d,{'files_loaded'})
%        file=char(d.files_loaded(1));
%        file_id=file(1:19);
%        cp=file(29:35);
%        file_id = [file_id '_' cp];
%    else
%        fprintf('File ID missing. Please enter date (by default, input missing)..\n');
%        calibrations=zeros(5,1); %don't do any calibration
%    end
%end
...
%
%   Relic, now algorithm name is an input.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%{
    Alt_chain_flag=0; % by defult we are not using the Alternative Chain.
 
    if strcmp(file_id(1:5),'luxsm') % If SIM, label as SIM
        algorithm_name='LUXkrypCal_SIM';
    else
...
if Pulse_Class==1
    %d.s1s2_pairing(isnan(d.s1s2_pairing))=0; %by default it returns nan
    %when there is no pair.... Removed, using golden now -AD
    d.z_drift_samples(isnan(d.z_drift_samples)) = 0.0; % get rid of NaN

   %Replaced by the golden varible ...
...
else %do a manual s1 s2 cut...
   %Cut based on prompt fraction (Kr 83)
    s1_index = d.pulse_area_phe>s1area_bound_min & d.pulse_area_phe<s1area_bound_max & d.prompt_fraction<1 & d.prompt_fraction>10^(-.3);
    %s1_index = d.pulse_area_phe>s1area_bound_min & d.pulse_area_phe<s1area_bound_max & d.exp_fit_tau_fall_samples <10^(1) & d.exp_fit_tau_fall_samples>10^(0.6);
    s2_index = d.pulse_area_phe>s2area_bound_min & d.pulse_area_phe<s2area_bound_max & d.prompt_fraction<10^(-1);

    events=sort(size(d.pulse_area_phe)); %The first element is the number of pulses. sometimes it's 5, sometimes it's 10

    %only select events that contain 1 S1 and 1 S2. And calculate dT
    s2_index_2=s2_index&((cumsum(s1_index)-s1_index)==1); % require S1 first
    single_s1_s2_phe = sum(s2_index_2,1)==1 & sum(s1_index,1)==1;
    %Finds the index of the S2 event in cases that only have one S1 and S2
    s2_single_cut = cumsum(s2_index_2).*s2_index_2==1 & repmat(single_s1_s2_phe,events(1),1); % use 5 not 10 for old rq1
    %Finds the index of the S1 event in cases that only have one S1 and S2
    s1_single_cut = cumsum(s1_index).*s1_index==1 & repmat(single_s1_s2_phe,events(1),1);

    %d.bottom=squeeze(sum(d.peak_area_phe(:,[61:120 122],:),2));%Bottom PMT area
    d.phe_bottom=d.pulse_area_phe./(1+d.top_bottom_ratio); %bottom PMT pulse area

    %clean_cut=( (d.pulse_area_phe(s1_single_cut)+d.pulse_area_phe(s2_single_cut) )'./...
    %d.full_evt_area_phe(logical(squeeze(sum(s2_single_cut,1)))) )>0.5;%more than 1/2 the area is S1+S2
    %dT micro seconds
    drift_time=(d.aft_t1_samples(s2_single_cut)-d.aft_t1_samples(s1_single_cut))*1e-2;%us
        %drift_distance_\mus=drift_time*drift_V; % units of \mus
    ...
    s2radius = (s2x.^2+s2y.^2).^(0.5);
        ...
        timestamp_vec = sort([d.livetime_latch_samples d.livetime_end_samples]);
    d.livetime_sec = sum(timestamp_vec(2:2:end) - timestamp_vec(1:2:end)) / 1e8;
    ...
    s2_width=(d.aft_t2_samples(s2_single_cut)-d.aft_t0_samples(s2_single_cut)); %cut above 800 samples
    %s1_width=d.aft_t2_samples(s1_single_cut)-d.aft_t0_samples(s1_single_cut); %length of S1, cut above 150 samples
    s1_width=d.pulse_end_samples(s1_single_cut)-d.pulse_start_samples(s1_single_cut); %length of S1, cut above 150 samples
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %   Calculate Single Electron Size 
    ...
    %Both PMT first fit
    SE_fit=fit([0:1:45]',hist(SE_phe_both(SE_radius<12),[0:1:45])','gauss1');
    SE_Size=SE_fit.b1;
    ...
    SE_cut=SE_phe_both(SE_radius<12);
    ...
    %Bottom SE Size
    SE_fit=fit([0:0.2:30]',hist(SE_phe_bot(SE_radius<12),[0:0.2:30])','gauss1');
    ...
    SE_cut_bot=SE_phe_bot(SE_radius<12);
    ...
    %%
    %Top SE Size
    SE_fit=fit([0:0.2:30]',hist(SE_phe_top(SE_radius<12),[0:0.2:30])','gauss1');
    ...
    SE_cut_top=SE_phe_top(SE_radius<12);

    [xxo, yyo, xo, no] = step(SE_cut_top(inrange(SE_cut_top,[fit_start fit_end])));
    s = fitoptions('Method','NonlinearLeastSquares','Robust','On','Algorithm','Trust-Region','Lower',[-5,0,0,0],'Upper',[5,Inf,Inf,Inf],...
        'Startpoint',[1,max(no),10,5]);
    skewGauss1 = fittype('a1/c1*exp(-((x-b1)/c1)^2)*(1+erf(a*(x-b1)/c1))','options',s);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    s2radius_d=s2radius;
    drift_time_d=drift_time;
    
    if length(s2radius) > 30000;
        index_perm= randperm(length(s2radius)); % randomly redistrubute the event numbers
        s2radius_d=s2radius(index_perm(1:30000),:); %cut out 30k events
        drift_time_d=drift_time(index_perm(1:30000),:); %cut out 30k events
    end
...
radial_field_fig=figure;
    densityplot(s2radius_d.^2,drift_time_d,'rectangle',[5,5],'lin');
...
if calibrations(1) == 1        
    ...
    bin_s1=5;%5
    x=100:bin_s1:500; %150:bin_s1:500;
    x=x';
...
%      time_cut = inrange(drift_distance_\mus,[binStart,binEnd]) & cut==1;
 ...
 % //////////////////////////////Make Plots/////////////////////////////////
    ...
    if length(2:plot_step:bins-1) == 6
            for i=[2:plot_step:bins-1,(bins-1)];  %first bin contains anode... skip it. start at i=2.
        xfit=100:2:500;
...
%     s1_z_correction=polyval(P_s1_both,241.6)./polyval(P_s1_both,drift_distance_\mus);%normalize s1 to 160 ns. 241.6 \mus
    s1_z_correction=polyval(P_s1_both,(det_edge-4)/2)./polyval(P_s1_both,drift_time);%normalize s1 to 160 ns. 241.6 \mus
...
%      time_cut = inrange(drift_distance_\mus,[binStart,binEnd]) & cut==1;
       time_cut = inrange(drift_time,[binStart,binEnd]) & cut==1; 
        ...
        temp_hist=hist_s2_both(:,bin);
            ...
            mean_start=sum(temp_hist.*x2)/sum(temp_hist);
            sigma_start=std(temp_hist.*x2);
        Fit_s2_both{bin}=fit(x2(2:end-1),temp_hist(2:end-1),'gauss1','start',[amp_start mean_start sigma_start]);%remove bin edges
...
%
%
%   Calculating the S2/S1 Z-dependence (i.e. electron lifetime)
%   Using S2_Bottom/S1_both
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     s1_z_correction=polyval(P_s1_both,241.6)./polyval(P_s1_both,drift_distance_\mus);%normalize s1 to 160 ns. 241.6 \mus
    ...
    clear y_s1_both y_s1_both Fit_s1_both Fit_EL xEL yfitEL xfit
    clear sigma_s1_both sigma_s1_both mean_index means mean_index means_error x temp_hist
    clear hist_s1_both dT_range dT_fit S x3
    
    %set up energy bins for the gaussian fit. Using both S2 only
   x3=0:0.1:35; %%for S2_b/S1
   x3=x3';
...
%     bin_size = (488/bins);%span the entire detector. 325us. typically = 5us
     bin_size = (floor(det_edge/bins*4)/4);%span the entire detector. 325us. typically = 5us. Round to nearest 0.25.
        ...
        hist_s2s1 = zeros(length(x3),bins);
        Fit_s2s1 = cell(1,bins);
        means=zeros(bins,1);
        means_error = zeros(bins,1);
        mean_index= zeros(bins,1);
...
%      time_cut = inrange(drift_distance_\mus,[binStart,binEnd]) & cut==1;
      time_cut = inrange(drift_time,[binStart,binEnd]) & cut;      
     
     %%calculate S2/S1 also, just to double check the lifetime from S2 only
      hist_s2s1(:,bin)= hist(s2_phe_bottom(time_cut)./s1_phe_both_z(time_cut),x3)';
      temp_hist=hist_s2s1(:,bin);  
            amp_start=max(temp_hist);
            mean_start=sum(temp_hist.*x3)/sum(temp_hist);
            sigma_start=std(temp_hist.*x3);
      Fit_s2s1{bin}=fit(x3(2:end-1),temp_hist(2:end-1),'gauss1','start',[amp_start mean_start sigma_start]);%remove bin edges        
      
      means(bin)=Fit_s2s1{bin}.b1;
      means_error(bin) = Fit_s2s1{bin}.c1/sqrt(2)/sqrt(sum(hist_s2s1(:,bin))); % == sigma/sqrt(N);
      mean_index(bin) = (binStart+binEnd)/2;     
 ...
 % //////////////////////////////Make Plots/////////////////////////////////
    ...
    e_lifetime_s2s1=-1/Fit_EL.b;
    s2s1_z0=Fit_EL.a;
       
    b=confint(Fit_EL, 0.683);%1 sigma matrix
    sigma_e_lifetime_s2s1=(1/b(1,2)-1/b(2,2))/2;
    sigma_s2s1_z0=(b(2,1)-b(1,1))/2;
 
    lifetime_fig_s2s1 = figure;
    errorbar(mean_index(2:end),means(2:end),means_error(2:end),'.k');  %first bin is anode, skip it
    hold on     
    plot(dT_fit,yfitEL,'-r','LineWidth',1);   
    xlabel('Time (\mus)', 'FontSize',16); ylabel('Mean S2_b/S1_z', 'FontSize', 16);
    title(strcat(file_id_cp,'.Kr83m. S2_b/S1_z Electron Lifetime'), 'FontSize', 16,'Interpreter','none');
    legend('S2_b/S1_z Mean', strcat( '\lambda= ', num2str(e_lifetime_s2s1,4), ' \pm ', num2str(sigma_e_lifetime_s2s1,2), '\mus' )...
        ,'location','northeast');
    myfigview(16);
    xlim([0 det_edge]);
    
    saveas(lifetime_fig_s2s1,strcat('LUX_corrections/',file_id_cp,'/S2S1_lifetime_',file_id_cp),'jpg');
    saveas(lifetime_fig_s2s1,strcat('LUX_corrections/',file_id_cp,'/S2S1_lifetime_',file_id_cp),'fig');
    
    %Plot 7 of the histograms     
    hist_s2s1_fig = figure;
    hold on
    plot_step = ceil((bins-1)/7);    
    j=1;
    t = {num2str(zeros(7,1))};
    if length(2:plot_step:bins-1) == 6 
            for i=[2:plot_step:bins-1,bins-1]  %first bin contains anode... skip it. start at i=2.
        xfit=.05:.01:30;
        yfit=Fit_s2s1{i}.a1.*exp(-((xfit-Fit_s2s1{i}.b1)./Fit_s2s1{i}.c1).^2);
        ...
        plot(x3,hist_s2s1(:,i)./d.livetime_sec,strcat(mark,color));
        plot(xfit,yfit./d.livetime_sec,color);
    ...
    for i=2:plot_step:bins-1  %first bin contains anode... skip it. start at i=2.
        ...
        title(strcat(file_id_cp,'.Kr83m. S2_b/S1_z'), 'FontSize', 16,'Interpreter','none');
        legend(strcat(t{1},' \mus'),strcat(t{1},' \mus Fit'),strcat(t{2},' \mus'),strcat(t{2},' \mus Fit')...
            ,strcat(t{3},' \mus'),strcat(t{3},' \mus Fit'),strcat(t{4},' \mus'),strcat(t{4},' \mus Fit')...
            ,strcat(t{5},' \mus'),strcat(t{5},' \mus Fit'),strcat(t{6},' \mus'),strcat(t{6},' \mus Fit')...
            ,strcat(t{7},' \mus'),strcat(t{7},' \mus Fit') ); 
    ...
    saveas(hist_s2s1_fig,strcat('LUX_corrections/',file_id_cp,'/S2S1_hist_EL_',file_id_cp),'jpg');
    saveas(hist_s2s1_fig,strcat('LUX_corrections/',file_id_cp,'/S2S1_hist_EL_',file_id_cp),'fig');
    
    s2s1_means=means;
    s2s1_means_sigma=means_error;
    s2s1_means_bin=mean_index;      
...
s2_bin=200;  
x2=5000:s2_bin:18000; % Set up energy bin size. Both PMT
x2=x2';
...
s1_phe_both_z=s1_phe_both.*s1_z_correction;
% s2_phe_both_z=s2_phe_both.*exp(drift_distance_mm/e_lifetime); %Correct S2 for lifetime
s2_phe_both_z=s2_phe_both.*exp(drift_time/e_lifetime);
   ...
   for x_bin=S2_xbin_min:S2xybinsize:(S2_xbin_max-S2xybinsize);
    ...
    for y_bin=S2_ybin_min:S2xybinsize:(S2_ybin_max-S2xybinsize);          
      ...
      y_min = y_bin; y_max = y_bin+S2xybinsize;      
       ...
       hist_bin = hist(s2_phe_both_z_2(bin_cut) ,x2)'/s2_bin; 
       Count_S2_both(y_count,x_count)=sum(hist_bin)*s2_bin; % X and Y flip ... because MATLAB
      ...
      if Count_S2_both(y_count,x_count)>30;                         
                ...
                Fit_S2_both_xy= fit(x2(2:end-1),hist_bin(2:end-1),'gauss1','start',[amp_start mean_start sigma_start]); %remove edges
...
s2_bin=50;   
x2=1000:s2_bin:10000; % Set up energy bin size. Bottom PMT
x2=x2';
...
s2_phe_both_z=s2_phe_both.*exp(drift_time/e_lifetime); %Correct S2 for lifeimte  %s2_phe_both_z[5000,18000]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for x_bin=S2_xbin_min:S2xybinsize:(S2_xbin_max-S2xybinsize);
    ...
    for y_bin=S2_ybin_min:S2xybinsize:(S2_ybin_max-S2xybinsize);          
      ...
      if Count_S1_both(y_count,x_count)>70; 
...
%     LUXkrypCal_S1_3D(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_distance_\mus,user_name,file_id_cp,dp_version,submit);
    LUXkrypCal_S1_3D(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_time,det_edge,user_name,file_id_cp,dp_version, algorithm_name, submit);
...
end %S2 width Calculations




%% save the data localy
    if ( calibrations(3) == 1 && calibrations(4) ==1 )
    save(strcat('LUX_corrections/',file_id_cp,'/',file_id_cp,'_IQs'),'e_lifetime','sigma_e_lifetime','Kr_events','P_s1_both','P_s1_bottom','sigma_P_s1_both','sigma_P_s1_bottom'...
        ...
        ,'s2_both_means_sigma','e_lifetime_s2s1','sigma_e_lifetime_s2s1', 's2s1_z0','sigma_s2s1_z0','s2s1_means','s2s1_means_bin','s2s1_means_sigma'...
        ,'mean_index_s2_z_width_full','means_s2_z_width_full','means_error_s2_z_width_full','mean_index_s2_z_width_aft1','means_s2_z_width_aft1'...
    ...
    elseif ( calibrations(1) == 1 && calibrations(2) == 1 )
       save(strcat('LUX_corrections/',file_id_cp,'/',file_id_cp,'_IQs'),'e_lifetime','sigma_e_lifetime','Kr_events','P_s1_both','P_s1_bottom','sigma_P_s1_both','sigma_P_s1_bottom'...
         ,'s1_both_means','s1_both_means_sigma','s1_both_means_bin','s1_bottom_means','s1_bottom_means_sigma','s1_bottom_means_bin'...
        , 's2_bottom_means','s2_bottom_means_sigma','s2_bottom_means_bin','file_id','file_id_cp','cp','s2_z0_both','delay_min'...
        ,'sigma_s2_z0_both','e_lifetime_both','sigma_e_lifetime_both','s2_z0_bottom','sigma_s2_z0_bottom','s2_both_means','s2_both_means_bin'...
        ,'s2_both_means_sigma','e_lifetime_s2s1','sigma_e_lifetime_s2s1', 's2s1_z0','sigma_s2s1_z0','s2s1_means','s2s1_means_bin','s2s1_means_sigma'...
        ,'P_s1_top','sigma_P_s1_top','det_edge'); 

    else
        %do nothing
        
    end%save variables locally 
...
%electron lifetime%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if calibrations(1)==1;
    ...
    lug_val(15) = {e_lifetime_s2s1};   %electron lifetime from S2b/S1z
...
%    lug_val(29) = {drift_V};            %Drift velocity used for e_lifetime
    ...
    Image_paths = {strcat('LUX_corrections/',file_id_cp,'/S2_bottom_lifetime_',file_id_cp,'.jpg'),strcat('LUX_corrections/',file_id_cp,'/S2_bottom_hist_EL_',file_id_cp,'.jpg')...
        ...
        strcat('LUX_corrections/',file_id_cp,'/S2S1_lifetime_',file_id_cp,'.jpg')}; 
...
%            xmlinput.iq.correction.fit.drift_velocity= lug_val(21);
    ...
    %S1 XYZ Correction%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Handeled by another function. LUXkrypCal_S1-3D.m
   ...
   %{ 
    ...
    lug_val(22) = {sigma_mean_center_3D_bottom}; 
    ...
    %strcat(file_id_cp,'/S1_3D_both_',file_id_cp,'.jpg')
            ...
            xmlinput.iq.global.gs_number = lug_val(3);
            xmlinput.iq.global.computed_by = lug_val(4);
            xmlinput.iq.global.computed_date = datestr(now,'yyyy\musddTHH\mus');
            xmlinput.iq.global.source = source;
            xmlinput.iq.global.source_id = 'unknown';
            xmlinput.iq.global.notes = ' ';
            xmlinput.iq.global.algorithm_name = lug_val(5);
            ...
            xmlinput.iq.correction.fit.drift_velocity= lug_val(24);
     
       LUXSubmitIQ(user,'s1_xyz_correction','LUXkrypCal',version,file_id,cp,xmlinput,'Kr83 Calibration','','');
...
%    lug_val(20) = {drift_V};
 ...
 %Single electron size %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if calibrations(6)==1;
    ...
    lug_val(13) = {SE_mu_bottom};
    lug_val(14) = {SE_sig_mu_bottom};
    lug_val(15) = {SE_sig_bottom};
    lug_val(16) = {SE_sig_sig_bottom};
    lug_val(17) = {skew_bottom};
