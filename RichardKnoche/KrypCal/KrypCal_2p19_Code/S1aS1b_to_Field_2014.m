%% Comparing S1a/S1b RvZ to Scott's field map in 2014
...
%                 s1b_xy_mean_err(y_count,x_count)=s1b_z_fit.c1/sqrt(2)/length(s1b_phe_both_z(bin_cut.' & inrange(s1b_phe_both_z,[0 300])));
    ...
    title(strcat('2014 S1a Mean vs. R^2 v Z.'),'fontsize',16,'Interpreter','none');
    ...
    xlabel('R^2 (cm^2)','fontsize', 18);
    ylabel('Z (uSec)','fontsize', 18);
    title(strcat('2014 S1b Mean vs. R^2 v Z.'),'fontsize',16,'Interpreter','none');
    caxis([color_range_min color_range_max])
    g=colorbar;
    ...
    xlabel('R^2 (cm^2)','fontsize', 18);
    ylabel('Z (uSec)','fontsize', 18);
    title(strcat('2014 S1a/S1b Mean vs. R^2 v Z.'),'fontsize',16,'Interpreter','none');
    caxis([color_range_min color_range_max])
    g=colorbar;
    ...
    contourf(s1ab_r2bins,s1ab_zbins,field_map_2014,vc,'LineColor','none');
    xlabel('R^2 (cm^2)','fontsize', 18);
    ylabel('Z (uSec)','fontsize', 18);
    title(strcat('2014 Field Value vs. R^2 v Z.'),'fontsize',16,'Interpreter','none');
    caxis([color_range_min color_range_max])
    g=colorbar;
    ...
    figure
    rkploterr(s1ab_r2z_mean(:),field_map_2014(:),s1ab_r2z_mean_err(:),field_map_error(:),[],[],[],2);
    ylabel('Field (V/cm)'); xlabel('S1a/S1b Ratio'); myfigview(16);
%% Fit a polynomial
% Call fminsearch with a random starting point.
cut= ~isnan(s1ab_r2z_mean) & ~isnan(field_map_2014) & s1ab_r2z_mean>0 & field_map_2014>0;
data.xdata=s1ab_r2z_mean(cut);
data.ydata=field_map_2014(cut);
data.xdataerr=s1ab_r2z_mean_err(cut);
data.ydataerr=field_map_error(cut);
[S1aS1b_fit_2014, S1aS1b_fit_2014_error]=polyfit(data.xdata,data.ydata,2); %given X
...
save('C:\Program Files\MATLAB\R2012a\bin\LUXCode\Scratch\RichardKnoche\KrypCal\KrypCal_2p19_Code\2014_Field_to_S1aS1b','S1aS1b_fit_2014','S1aS1b_fit_2014_error');