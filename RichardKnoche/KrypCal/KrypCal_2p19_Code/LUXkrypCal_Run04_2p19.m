...
%     s1_z_correction=polyval(P_s1_both,241.6)./polyval(P_s1_both,drift_distance_\mus);%normalize s1 to 160 ns.
    ...
    %% Gaussian fit to s1_phe_both_z (which is field and z-efficiency corrected)
    
    s1_phe_both_z_gauss_fit=fit([0:10:1000].',hist(s1_phe_both_z(inrange(s1_phe_both_z,[0 1000])),[0:10:1000]).','gauss1');
...
%% S2 phe both xyz gauss fit
s2_phe_both_xyz=s2_phe_both_z.*interp2(s2xbins,s2ybins,norm_S2_all,s2x,s2y,'spline',1);
s2_phe_both_xyz_gauss_fit=fit([0:100:30000].',hist(s2_phe_both_xyz(inrange(s2_phe_both_xyz,[0 30000])),[0:100:30000]).','gauss1');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for x_bin=S2_xbin_min:S2xybinsize:(S2_xbin_max-S2xybinsize);
    ...
    for y_bin=S2_ybin_min:S2xybinsize:(S2_ybin_max-S2xybinsize);          
      ...
      if Count_S2_bottom(y_count,x_count)>350; % At least 40 events for a gaussian fit. Else no good
...
%% S1 xyz gauss fit
s1_phe_both_xyz=s1_phe_both_z.*interp2(s1xbins,s1ybins,norm_S1_all,s2x,s2y,'spline',1);
s1_phe_both_xyz_gauss_fit=fit([0:10:1000].',hist(s1_phe_both_xyz(inrange(s1_phe_both_xyz,[0 1000])),[0:10:1000]).','gauss1');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for x_bin=S2_xbin_min:S2xybinsize:(S2_xbin_max-S2xybinsize);
    ...
    for y_bin=S2_ybin_min:S2xybinsize:(S2_ybin_max-S2xybinsize);          
      ...
      if Count_S1_bottom(y_count,x_count)>350; 
...
%     LUXkrypCal_S1_3D(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_distance_\mus,user_name,file_id_cp,dp_version,submit);
    LUXkrypCal_S1_3D_Run04_2p19(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_time,x_center,y_center,z_center,det_edge,inpaint_on,user_name,file_id_cp,dp_version, algorithm_name, submit);
