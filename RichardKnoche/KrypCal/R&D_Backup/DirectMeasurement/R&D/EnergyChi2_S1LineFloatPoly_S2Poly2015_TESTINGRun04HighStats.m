...
hold on;
plot([10:1:360],polyval(s1ab_P,[10:1:360]),'-r','LineWidth',2)
...
s1ab_xbin_max=25;
s1ab_ybin_max=25;
s1ab_xybinsize=sqrt((50*50)/(length(s1ab_z(s1ab_z<330))/300));
clear s1a_xy_mean s1a_xy_mean_err s1b_xy_mean s1b_xy_mean_err s1ab_xy_mean s1ab_xy_mean_err
...
s1b_xy_mean=zeros(length(s1ab_xbins),length(s1ab_ybins));
 ...
 for x_bin=s1ab_xbin_min:s1ab_xybinsize:(s1ab_xbin_max-s1ab_xybinsize/2);
        for y_bin=s1ab_ybin_min:s1ab_xybinsize:(s1ab_ybin_max-s1ab_xybinsize/2);          
          ...
          bin_cut = s1ab_z_cut.' & s1a_phe_both>0 & s1b_phe_both>0 & inrange(s1ab_x,[x_min,x_max]).' & inrange(s1ab_y,[y_min,y_max]).'; 
...
%                 s1ab_z_fit=fit([0:0.1:10].',hist(s1a_phe_both_z(bin_cut.' & inrange(s1b_phe_both_z,[0 300]))./s1b_phe_both_z(bin_cut.' & inrange(s1b_phe_both_z,[0 300])),[0:0.1:10]).','gauss1');
%                 s1ab_xy_mean(y_count,x_count)=s1ab_z_fit.b1;
%                 s1ab_xy_mean_err(y_count,x_count)=s1ab_z_fit.c1/sqrt(2)/length(s1b_phe_both_z(bin_cut.' & inrange(s1b_phe_both_z,[0 300])));
            else
...
%                 s1ab_xy_mean(y_count,x_count)=0;
%                 s1ab_xy_mean_err(y_count,x_count)=0;
            end
...
s1ab_xyz_numbins=floor((length(s1ab_z)/300)^(1/3)); %number of bins in one direction
...
s1ab_s1_xyz_fit_map.p1=0;
s1ab_s1_xyz_fit_map.p2=0.5;
...
% s1ab_xy_mean(s1ab_xy_mean==0)=nan; s1ab_xy_mean=inpaint_nans(s1ab_xy_mean);
...
s2zbins3D=floor(10):z_step:floor(10)+z_step*10; %20 us

Count_S2_3D=zeros(10,10,11);
Count_S2_3D_bottom=zeros(10,10,11);
Mean_S2_3D_both=zeros(10,10,11);
Mean_S2_3D_bottom=zeros(10,10,11);
Sigma_S2_3D_both=zeros(10,10,11);
Sigma_S2_3D_bottom=zeros(10,10,11);

for k = s2zbins3D; % out to 485 mm % start at about 20 mm bin center. xy_steps of 30 mm
    for i = (-r_max+xy_step):xy_step:r_max %map to rows going down (y_cm)
        for j = (-r_max+xy_step):xy_step:r_max %map columns across (x_cm)
           ...
           %sort, and make the cut. using the variable q
           q = inrange(s2_phe_both,[1000,40000]) & s2x<j & s2x>(j-xy_step) & s2y<i & s2y>(i-xy_step) & inrange(drift_time,[(k-z_step/2) , (k+z_step/2)]); %no 0th element!
                    
           % Hist 
            hist_q = hist(s2_phe_both(q) ,x2)'/bin_s2;
                        
            %Count the number of events per bin
            Count_S2_3D(l,m,n)=sum(hist_q)*bin_s2;
            
            if (Count_S2_3D(l,m,n) >= 30) % at least 30 counts before fitting. 
                
                yqfit=hist_q;
                    amp_start=max(yqfit);
                    mean_start=sum(yqfit.*x2)/sum(yqfit);
                    sigma_start=std(yqfit.*x2);    
                Fit_q= fit(x2,yqfit,'gauss1','start',[amp_start mean_start sigma_start]);
                
                %Peak location (mean)
                Mean_S2_3D_both(l,m,n)=Fit_q.b1;
                
                %1-sigma of peak position
                Sig_b=Fit_q.c1/sqrt(2)/sqrt(Count_S2_3D(l,m,n));
...
s2xbins3D=-r_max+xy_step/2:xy_step:r_max-xy_step/2;
s2ybins3D=-r_max+xy_step/2:xy_step:r_max-xy_step/2;
...
center_phe_both_xyz= RK_1DCubicInterp_LinearExtrap(s2_both_norm_z_index,s2_both_norm_z_means,4); %interp3(s2xbins3D,s2ybins3D,s2zbins3D,Mean_S2_3D_both,x_center,y_center,4,'cubic');%Normalize to the center (x=y=0. dT=160us)
norm_s2_both_xyz=center_phe_both_xyz./Mean_S2_3D_both; %Normalize to center (x=y=0. dT=160)
norm_s2_both_xyz(isinf(norm_s2_both_xyz))=1;%remove infinity. no correction outside 25cm
norm_s2_both_xyz(isnan(norm_s2_both_xyz))=1;%remove nan. no correction outside 25cm

s2_3Dcorr=interp3(s2xbins3D,s2ybins3D,s2zbins3D,norm_s2_both_xyz,s2x,s2y,drift_time,'spline',1);
...
s1ybins=(S1_ybin_min+S1xybinsize/2):S1xybinsize:(S1_ybin_max-S1xybinsize/2);
    ...
    xlabel('x (cm)','fontsize', 18);
    ylabel('y (cm)','fontsize', 18);
    title(strcat(file_id_cp, '. S1 Mean vs. XY.'),'fontsize',16,'Interpreter','none');
    caxis([color_range_min color_range_max])
    g=colorbar;
    ylabel(g,'S1 Mean (phe)','FontSize',16)
    set(g,'FontSize',18)
    hold on
    LUXPlotTopPMTs
    axis([-25 25 -25 25])
...
%% Plots
figure
plot(s1_3Dcorr(~(s1_3Dcorr==1)),(s1_z_correction(~(s1_3Dcorr==1)).*s1xy_correction(~(s1_3Dcorr==1))),'.k')
hold on;
plot(s1_3Dcorr(~(s1_3Dcorr==1)),s1_3Dcorr(~(s1_3Dcorr==1)),'-r')
xlabel('s1 3D correction');ylabel('s1 XY+Z correction'); myfigview(16);
title('Run04 - Highstats LT 30');

%% Plots
figure
plot(s2_3Dcorr(~(s2_3Dcorr==1)),(s2z_correction(~(s2_3Dcorr==1)).*s2xy_correction(~(s2_3Dcorr==1))),'.k')
hold on;
plot(s2_3Dcorr(~(s2_3Dcorr==1)),s2_3Dcorr(~(s2_3Dcorr==1)),'-r')
xlabel('s2 3D correction');ylabel('s2 XY+Z correction'); myfigview(16);
title('Run04 - Highstats LT 30');