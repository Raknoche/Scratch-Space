...
%  path='C:\Users\Richard\Desktop\TestData\CH3T';
 path='C:\Users\Richard\Desktop\TestData\2.18 IQs\lux10_20150122T1337';
...
rqs_to_load = {'pulse_area_phe','event_timestamp_samples'...
   ...
   'correction_s2_z_dependence','correction_s1_z_dependence',...
   'correction_s1_xy_dependence',...
   'correction_s1_xyz_dependence','correction_s2_xy_dependence'};
...
%plot the s2xy+z and s2z corr
figure
hold on;
plot(drift_time,s2zcorr.*s2xycorr,'.r')
plot(drift_time,s2zcorr,'.k')
xlabel('Drift time'); ylabel('S2 correction');

% %plot the s1xy+z and s2z corr
% figure
% hold on;
% plot(drift_time,s1zcorr.*s1xycorr,'.r')
% plot(drift_time,s1zcorr,'.k')
% xlabel('Drift time'); ylabel('S1 correction');

%Location of negative s2 events
figure
plot(s2radius(s2zcorr<1),drift_time(s2zcorr<1),'.k');
xlabel('Radius(cm)');ylabel('Drift Time (uSec)');
title('Less than One Corrected S2 events');
myfigview(16);
set(gca,'Ydir','reverse')