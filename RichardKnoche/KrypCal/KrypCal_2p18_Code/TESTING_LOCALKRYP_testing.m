...
%%
    ...
    if Kr_events < 30000 %then create a 10x10 grid. Otherwise 25x25. Want about 30 evts per bin
            ...
            S1xybinsize = sqrt((50*50)/(Kr_events/100));
            S1_xbin_min = -25;
            S1_xbin_max = 25;
            S1_ybin_min = -25;
            S1_ybin_max = 25;
            ...
            S2xybinsize = sqrt((50*50)/(Kr_events/100));%cm
...
s1ab_xbin_max=25;
s1ab_ybin_max=25;
s1ab_xybinsize=sqrt((50*50)/(length(s1ab_z)/400));
...
s1ab_xy_mean(s1ab_xy_mean==0)=nan;
s1ab_xy_mean=inpaint_nans(s1ab_xy_mean);
...
%%Plot the norm_S2_all

color_range_max=max(max(norm_S2_all));
color_range_min=min(min(norm_S2_all(norm_S2_all>0)));
vc_step=(color_range_max-color_range_min)/50;

vc=color_range_min:vc_step:color_range_max;
 
sigma_s2_xy_both_fig = figure;
contourf(s2xbins,s2ybins,norm_S2_all,vc,'LineColor','none');
xlabel('x (cm)','fontsize', 18);
ylabel('y (cm)','fontsize', 18);
title(strcat(file_id_cp, '. normS2all(Both PMT) vs. XY.'),'fontsize',16,'Interpreter','none');
caxis([color_range_min color_range_max])
g=colorbar;
ylabel(g,'phe','FontSize',16)
set(g,'FontSize',18)
