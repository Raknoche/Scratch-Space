function [ dp ] = Apply_TempCorrections_2p15(dp,file_name,OutagePath,tzoffset)
...
current_data_set_time=filename2epoch_krypcal(filename_prefix,tzoffset); %I think this is hard coded to some other time zone? +6 hours from MT?
...
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "electron_lifetime" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
           ...
           iq_times = [iq_times filename2epoch_krypcal(value1.iq.global.filename_prefix,tzoffset) ];
 ...
 [s2_norm_z s2_norm]=InterpS2IQ_Run04(current_data_set_time, iq_times, s2_norm_z_iqs, s2_norm_iqs, noflow_times);
...
clear out
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "z_dep_s1_correction" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
   ...
   [index iq_time_zDep] = NearestIQ_Run04(current_data_set_time,iq_times,noflow_times);
...
clear out
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "s2_xy_correction" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
   ...
   [index iq_time_s2xyDep] = NearestIQ_Run04(current_data_set_time,iq_times,noflow_times);
...
clear out
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "s1_xy_correction" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
   ...
   [index iq_time]  = NearestIQ_Run04(current_data_set_time,iq_times,noflow_times);
    clear value1
    value1 = XMLParser(out.values_xml{index});      
...
clear out
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "s1_xyz_correction" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
   ...
   [index iq_time_s1xyzDep] = NearestIQ_Run04(current_data_set_time,iq_times,noflow_times);
...
clear out
out = MySQLQuery('select values_xml from control.lug_iqs where iq_type = "s1ab_xy" and strikeme = 0 and algorithm_version = 2.15;');
...
value1 = XMLParser(out.values_xml{j});
   ...
   [index iq_time_det_center] = NearestIQ_Run04(current_data_set_time,iq_times,noflow_times);
    clear value1
   value1 = XMLParser(out.values_xml{index});      
