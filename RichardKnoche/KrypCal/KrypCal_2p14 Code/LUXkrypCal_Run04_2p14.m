...
%Set up cuts and variables
    ...
    s1ab_x=s1ab_x(s1ab_areacut);
    s1ab_y=s1ab_y(s1ab_areacut);
    s1ab_z=s1ab_z(s1ab_areacut);
    s1a_phe_both=s1a_phe_both(s1ab_areacut);
    s1b_phe_both=s1b_phe_both(s1ab_areacut);
...
if length(s1ab_z)>150000; %require 100,000 events to do 3D binning
...
% load('/scratch4/UMDcode/S1aS1bfieldmapping.mat');
 ...
 s1ab_s2_xyz_fit_map.p1=-0.76;
 s1ab_s2_xyz_fit_map.p2=2.45;
 s1ab_s2_xyz_fit_map.p3=1- s1ab_s2_xyz_fit_map.p1.*2.755.^2-2.755.* s1ab_s2_xyz_fit_map.p2;
 
 s1ab_s1_xyz_fit_map.p1=0;
 s1ab_s1_xyz_fit_map.p2=0.9;
 s1ab_s1_xyz_fit_map.p3=1-s1ab_s1_xyz_fit_map.p1.*2.755.^2-2.755.* s1ab_s1_xyz_fit_map.p2;
 
 %If we can use 3D map, do so.  If there aren't enough events do 1D then 2D
 if length(s1ab_z)>150000; %require 100,000 events to do 3D binning
 %interpolating the s1a/s1b values at all s2x and s2y positions
 if inpaint_on==1;  s1ab_xyz_mean(s1ab_xyz_mean==0)=nan; s1ab_xyz_mean=inpaint_nans3(s1ab_xyz_mean,0); end;
 s1as1b_values=interp3(s1ab_xyz_xbins,s1ab_xyz_ybins,s1ab_xyz_zbins,s1ab_xyz_mean,s2x,s2y,drift_time,'spline');
 s1as1b_at_center=interp3(s1ab_xyz_xbins,s1ab_xyz_ybins,s1ab_xyz_zbins,s1ab_xyz_mean,x_center,y_center,z_center,'spline');
...
s1_phe_both=s1_phe_both./s1_3Dfieldremoval;
 ...
 %interpolating the s1a/s1b values at all s2x and s2y positions
 if inpaint_on==1;  s1ab_xy_mean(s1ab_xy_mean==0)=nan; s1ab_xy_mean=inpaint_nans(s1ab_xy_mean,3); end;
 s1as1b_zvalues= polyval(s1ab_P,drift_time);
 s1as1b_zcorr_xyvalues=interp2(s1ab_xbins,s1ab_ybins,s1ab_xy_mean,s2x,s2y,'spline');
 s1as1b_z_at_center=polyval(s1ab_P,z_center); 
 s1as1b_xy_at_center=interp2(s1ab_xbins,s1ab_ybins,s1ab_xy_mean,x_center,y_center,'spline');
...
%      time_cut = inrange(drift_distance_\mus,[binStart,binEnd]) & cut==1;
 ...
 % //////////////////////////////Make Plots/////////////////////////////////
    ...
    dT_range= inrange( mean_index,[50,300] );%from 50 to 300us for the fit
    dT_fit=0:1:det_edge;    
...
%      time_cut = inrange(drift_distance_\mus,[binStart,binEnd]) & cut==1;
 ...
 % //////////////////////////////Make Plots/////////////////////////////////
    ...
    dT_range= inrange( mean_index,[50, 300] );%from 50 to 300us for the fit, based on when s1as1b values get strange
    dT_fit=0:1:det_edge;    
...
%     LUXkrypCal_S1_3D(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_distance_\mus,user_name,file_id_cp,dp_version,submit);
    LUXkrypCal_S1_3D_Run04_2p14(s1_phe_both,s1_phe_bottom,s2_phe_both,s2x,s2y,drift_time,x_center,y_center,z_center,det_edge,inpaint_on,user_name,file_id_cp,dp_version, algorithm_name, submit);
